...
# Call the function
# div_by(10,2)
# print(div_by(10,2)) # print output
...
def sum_odd(a, b):
    """"
    input: a,b(int)
    Find the sum of all odd numbers between(a and b included)
    """
    sum_of_odds = 0
...
# print(keep_consonants("abcdefgh"))
...
def first_to_last_diff(s, c):
    ...
    if not c in s:
        return -1
    ## reach here if c in s
    for i in range(len(s)):
        if s[i] == c:
            # break to return the first instance of c in s
            break
    for j in range(len(s)-1, -1, -1):
        if s[j] == c:
            # break to return second instance
            break
    return j-i
